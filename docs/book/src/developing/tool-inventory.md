...
| `claude_code`, `claude_code_runner`, `codex_cli`, `gemini_cli`, `opencode_cli`, `grok_cli` | CLI-backed integration or skill package. | The external CLI already owns authentication, command behavior, and release cadence; ZeroClaw should preserve receipts and policy if it invokes them. |
